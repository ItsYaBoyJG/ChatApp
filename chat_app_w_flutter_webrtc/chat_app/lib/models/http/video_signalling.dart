...
class Signaling {
  ...
  void invite(String peerId, String media, bool useScreen) async {
    ...
    onCallStateChange?.call(session, CallState.newState);
    onCallStateChange?.call(session, CallState.inviteState);
  }
      ...
      case 'offer':
        {
          ...
          await newSession.peerConnection?.setRemoteDescription(
              RTCSessionDescription(description['sdp'], description['type']));

          if (newSession.remoteCandidates!.isNotEmpty) {
            newSession.remoteCandidates!.forEach((candidate) async {
              await newSession.peerConnection?.addCandidate(candidate);
            });
            newSession.remoteCandidates!.clear();
          }
          onCallStateChange?.call(newSession, CallState.newState);
          onCallStateChange?.call(newSession, CallState.ringState);
        }
        break;
      case 'answer':
        {
          ...
          session?.peerConnection?.setRemoteDescription(
              RTCSessionDescription(description['sdp'], description['type']));
          onCallStateChange?.call(session!, CallState.connectedSate);
        }
        break;
      case 'candidate':
        {
      ...
      case 'bye':
        {
          ...
          if (session != null) {
            onCallStateChange?.call(session, CallState.hangUpState);
  ...
  Future<void> connect() async {
    ...
    _socket = HttpSocket(url);

    if (_turnCredential == null) {
      try {
        _turnCredential = await _credentials.getTurnCredential(_host, _port);
        _iceServers = {
          'iceServers': [
            {
